...
<body>
    ...
    if (!empty($_POST['amount'])) {
        ...
        if ($current_balance >= $amount) {
            ...
            if ($stmt->execute()) {
                ...
                $transaction_sql = "INSERT INTO " . $transaction_table . " (transid, trans_type, trans_date, `trans-amount`, lastname, firstname, phone) VALUES (?, 'Withdrawal', NOW(), ?, ?, ?, ?)";
                $transaction_stmt = $conn->prepare($transaction_sql);
                $transaction_stmt->bind_param("sdsss", $account_number, $amount, $lastname, $firstname, $phone);
                $transaction_stmt->execute();

                echo "<p>Withdrawal successful! New balance: $$new_balance</p>";
            } else {
                echo "<p>Error: " . $conn->error . "</p>";
            }
            $stmt->close();
        } else {
            echo "<p>Insufficient funds. Current balance: $$current_balance</p>";
        }
        $balance_stmt->close();
    }
    ...
    $conn->close();
    ?>
    <form action="withdraw.php?account_type=<?php echo $account_type; ?>&account_number=<?php echo $account_number; ?>" method="POST">
